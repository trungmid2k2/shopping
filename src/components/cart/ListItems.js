...
export default function ListItems() {
  ...
  const listCart = items.map((item, index) => {
    const ttprice1product = currency(item.price * item.quantity);
    const ttasspussy = ttprice1product.format();
    ...
    return (
      <li key={item.id} className="flex items-center my-3 py-[10px]">
        ...
        <div className="w-[150px] flex items-center justify-center">
          <button
            className="w-[30px] border-[#a9a9a9] border text-[16px] hover:bg-[#dfdfdf]"
            onClick={() => handleDecreaseQuantity(item.id)}
          >
            -
          </button>

          <div className="px-4 border-y border-y-[#a9a9a9]">
            {item.quantity}
          </div>
        ...
        <div className="w-[150px]">
          {ttasspussy} (<span>${item.price}</span>)
        </div>
